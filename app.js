...
let currentParts=[];
function partsStockTotal(list){return (list||[]).filter(x=>!x.external).reduce((a,x)=>a+(+x.qty||0)*(+x.sell||0),0)}
function partsStockCost(list){return (list||[]).filter(x=>!x.external).reduce((a,x)=>a+(+x.qty||0)*(+x.cost||0),0)}
function partsExternalProfit(list){return (list||[]).filter(x=>x.external).reduce((a,x)=>a+(+x.qty||0)*((+x.sell||0)-(+x.cost||0)),0)}
...
function renderOrderParts(){let el=document.getElementById("orderParts");el.innerHTML=currentParts.map((x,i)=>{let p=x.external?null:arr(K.p).find(z=>z.id===x.partId);let nm=x.external?(x.name||"قطعة خارجية"):(p?.name||"قطعة محذوفة");let amount=x.external?((+x.sell||0)-(+x.cost||0))*x.qty:x.qty*x.sell;let amountLabel=x.external?`${amount.toFixed(2)} ج مكسب`:`${amount.toFixed(2)} ج`;return `<div class="part-row${x.external?" part-row-external":""}"><span>${x.external?"🧳 ":""}${esc(nm)}${x.external?' <small class="ext-badge">خارج المخزن • يُضاف للمصنعية</small>':""}</span><input type="number" min="1" value="${x.qty}" onchange="currentParts[${i}].qty=+this.value;calc();renderOrderParts()"><span title="${x.external?`شراء ${(+x.cost||0).toFixed(2)} ج / بيع ${(+x.sell||0).toFixed(2)} ج`:""}">${amountLabel}</span><button type="button" class="secondary" onclick="currentParts.splice(${i},1);renderOrderParts();calc()">🗑️</button></div>`}).join("")}
function calc(){let ps=partsStockTotal(currentParts),extProfit=partsExternalProfit(currentParts),t=ps+(+rLabor.value||0)+extProfit,dep=+rDeposit.value||0;rPartsTotal.value=ps.toFixed(2);if(document.getElementById("rExternalProfit"))document.getElementById("rExternalProfit").value=extProfit.toFixed(2);rTotal.value=t.toFixed(2);remainBox.classList.toggle("hidden",dep<=0);rRemain.value=Math.max(0,t-dep).toFixed(2)}
...
function confirmAddExternalPartToRequest(requestId){let rs=arr(K.r),r=rs.find(x=>x.id===requestId);if(!r)return alert("أمر الشغل غير موجود.");if(r.closed||r.paid)return alert("الأمر مغلق أو مدفوع بالكامل ولا يمكن إضافة قطع غيار.");let nameEl=document.getElementById("rpExtName"),buyEl=document.getElementById("rpExtBuy"),sellEl=document.getElementById("rpExtSell"),qtyEl=document.getElementById("rpExtQty");let name=(nameEl?.value||"").trim();if(!name)return alert("اكتب اسم القطعة.");let cost=+(buyEl?.value||0),sell=+(sellEl?.value||0),q=+(qtyEl?.value||1);if(!Number.isFinite(q)||q<1)q=1;if(!Number.isFinite(cost)||cost<0||!Number.isFinite(sell)||sell<0)return alert("اكتب أسعار صحيحة.");let updatedParts=(r.parts||[]).map(x=>({...x}));updatedParts.push({external:true,name,qty:q,sell,cost});let addedProfit=q*(sell-cost),laborTotal=(+r.labor||0)+addedProfit,partsTotal=partsStockTotal(updatedParts),partsCost=partsStockCost(updatedParts),total=laborTotal+partsTotal;let updated={...r,parts:updatedParts,labor:laborTotal,partsTotal,partsCost,total,remain:Math.max(0,total-(+r.deposit||0))};try{put(K.r,rs.map(x=>x.id===r.id?updated:x))}catch(e){alert("تعذر حفظ إضافة القطعة: "+(e?.message||e));return}if(nameEl)nameEl.value="";if(buyEl)buyEl.value="";if(sellEl)sellEl.value="";if(qtyEl)qtyEl.value=1;requestProfile()}
